...
{
	printf(_("%s resynchronizes a Halo cluster with another copy of the cluster.\n\n"), progname);
	printf(_("Usage:\n  %s [OPTION]...\n\n"), progname);
	printf(_("Options:\n"));
...
main(int argc, char **argv)
{
	...
	if (argc > 1)
	{
		...
		{
			puts("pg_rewind (Halo) " PG_VERSION_PREFIX PG_VERSION);
			exit(0);
		}
	}
	...
	if (geteuid() == 0)
	{
		...
		fprintf(stderr, _("You must run %s as the Halo superuser.\n"),
				progname);
		exit(1);
	}
#endif
