/*-------------------------------------------------------------------------
 *
 * initdb --- initialize a Halo installation
 *
 * initdb creates (initializes) a Halo database cluster (site,
 * instance, installation, whatever).  A database cluster is a
 * collection of Halo databases all managed by the same server.
 *
 ...
 * Halo cluster location, and should be highly compatible with it.
...
"# allows any local user to connect as any Halo user, including\n" \
...
choose_dsm_implementation(void)
{
	...
	while (ntries > 0)
	{
		...
		handle = random();
		snprintf(name, 64, "/Halo.%u", handle);
...
bootstrap_template1(void)
{
	...
	snprintf(headerline, sizeof(headerline), "# Halo %s\n",
			 PG_MAJORVERSION);

	if (strcmp(headerline, *bki_lines) != 0)
	{
		pg_log_error("input file \"%s\" does not belong to Halo %s",
					 bki_file, PG_VERSION);
		fprintf(stderr,
...
{
	printf(_("%s initializes a Halo database cluster.\n\n"), progname);
	printf(_("Usage:\n"));
...
main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		{
			puts("initdb (Halo) " PG_VERSION_PREFIX PG_VERSION);
			exit(0);
		}
	}
