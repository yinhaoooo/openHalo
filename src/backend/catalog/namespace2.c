 ...
 *
 * namespace2.c
 *		Extra routines for namespace operations
 *
 * 
 ...
 *
 * IDENTIFICATION
 *	  src/backend/catalog/namespace2.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * get_specific_namespace_oid_by_env - return the specific OID
 * - PG_CATALOG_NAMESPACE: default
 * - "oracle": database_compat_mode = 'oracle'
 * - "sybase": database_compat_mode = 'sybase'
 * - "db2": database_compat_mode = 'db2'
 * - "sys": database_compat_mode = 'sqlserver'
 * - "mysql": database_compat_mode = 'mysql'
 */
Oid
get_specific_namespace_oid_by_env(void)
{
    Oid     oid;
    ...
    switch (database_compat_mode)
    {
		...
		case MYSQL_COMPAT_MODE:
		{
			...
			/* char	*mydb_name = get_database_name(MyDatabaseId); */
			/* char	*mydb_name = MyProcPort->database_name; */
			/* oid = get_namespace_oid(mydb_name, true); */
            if ((MyProcPort != NULL) && 
