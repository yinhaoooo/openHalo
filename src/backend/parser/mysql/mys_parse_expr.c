...
static Const *mys_make_const(ParseState *pstate, Value *value, int location);
		...
		case T_A_Const:
			{
				...
				result = (Node *) mys_make_const(pstate, val, con->location);
				break;
			}
...
transformColumnRef(ParseState *pstate, ColumnRef *cref)
{
	...
	if (node == NULL)
	{
		node = finalTransformColumnRef(pstate, cref);
	}
...
transformUserVarRef(ParseState *pstate, UserVarRef *userVarRef)
{
    ...
    n = makeNode(A_Const);
	n->val.type = T_String;
	n->val.val.str = userVarRef->userVarName;
...
transformUserVarAssign(ParseState *pstate, UserVarAssign *userVarAssign)
{
	...
	n = makeNode(A_Const);
	n->val.type = T_String;
	n->val.val.str = userVarAssign->userVarName;
	...
	{
		Node *expr = copyObject(userVarAssign->expr);
		Oid targetTypOid;
		int32 targetTypMod;
		Node *tempResult;

		tempResult = transformExprRecurse(pstate, expr);

		targetTypOid = exprType(tempResult);
		targetTypMod = exprTypmod(tempResult);

		result = coerce_to_target_type(pstate, result, exprType(result),
									targetTypOid, targetTypMod,
									COERCION_EXPLICIT,
...
transformSysVarRef(ParseState *pstate, SysVarRef *sysVarRef)
{
    ...
    if (strncasecmp(sysVarRef->sysVarName, "local.", 6) == 0)
    {
        A_Const *n = makeNode(A_Const);
        n->val.type = T_String;
        n->val.val.str = sysVarRef->sysVarName + 6;
        n->location = sysVarRef->location;
        funcCall = (Node *) makeFuncCall(list_make1(makeString("get_system_session_variable")),
                                         list_make1(n),
                                         COERCE_EXPLICIT_CALL,
                                         -1);
    }
    else if (strncasecmp(sysVarRef->sysVarName, "session.", 8) == 0)
    {
        A_Const *n = makeNode(A_Const);
        n->val.type = T_String;
        n->val.val.str = sysVarRef->sysVarName + 8;
        n->location = sysVarRef->location;
        funcCall = (Node *) makeFuncCall(list_make1(makeString("get_system_session_variable")),
                                         list_make1(n),
                                         COERCE_EXPLICIT_CALL,
    ...
    else if (strncasecmp(sysVarRef->sysVarName, "global.", 7) == 0)
    {
        A_Const *n = makeNode(A_Const);
        n->val.type = T_String;
        n->val.val.str = sysVarRef->sysVarName + 7;
        n->location = sysVarRef->location;
        funcCall = (Node *) makeFuncCall(list_make1(makeString("get_system_global_variable")),
                                         list_make1(n),
                                         COERCE_EXPLICIT_CALL,
                                         -1);
    }
    else 
    {
        A_Const *n = makeNode(A_Const);
        n->val.type = T_String;
        n->val.val.str = sysVarRef->sysVarName;
        n->location = sysVarRef->location;
        funcCall = (Node *) makeFuncCall(list_make1(makeString("get_system_session_variable")),
                                         list_make1(n),
                                         COERCE_EXPLICIT_CALL,
...
transformAExprOp(ParseState *pstate, A_Expr *a)
{
	...
	else
	{
        ...
        result = (Node *) mys_make_op(pstate,
                                      a->name,
                                      lexpr,
                                      rexpr,
                                      last_srf,
...
transformAExprNullIf(ParseState *pstate, A_Expr *a)
{
    ...
    result = (OpExpr *) mys_make_op(pstate,
                                    a->name,
                                    lexpr,
                                    rexpr,
                                    pstate->p_last_srf,
...
transformBoolExpr(ParseState *pstate, BoolExpr *a)
{
	...
	foreach(lc, a->args)
	{
		...
		if (inputType != BOOLOID)
		{
			/* 
			 * 当布尔表达式的参数中有非布尔值，将其做如下处理
			 * arg::float8 <> 0
			 */
			Node *lexpr = NULL;
			Const *rexpr = NULL;

			lexpr = coerce_to_target_type(pstate, arg, inputType,
										FLOAT8OID, -1,
										COERCION_EXPLICIT,
										COERCE_EXPLICIT_CAST,
										-1);
 ...
 */
static Const *
mys_make_const(ParseState *pstate, Value *value, int location)
{
	Const	   *con;
	Datum		val;
	int64		val64;
		...
		case T_ByteaString:
            {
                ...
                int bytSize = 0;
                int dataLen = 0;
                dataLen = atoi(value->val.str);
                bytSize = dataLen + VARHDRSZ;
                byt = palloc(bytSize);
                SET_VARSIZE(byt, bytSize);   /* actual length */
                memcpy(VARDATA(byt), (value->val.str + 16), dataLen);
                val = PointerGetDatum(byt);
                typeid = BYTEAOID;
                typelen = -1;
                typebyval = false;
                break;
