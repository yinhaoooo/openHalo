...
enum_in(PG_FUNCTION_ARGS)
{
        ...
        (nodeTag(MyProcPort->protocol_handler) == T_MySQLProtocol))
    {
        ...
        catlist = SearchSysCacheList1(ENUMTYPOIDNAME, ObjectIdGetDatum(enumtypoid));
        ...
        for (i = 0; i < catlist->n_members; i++)
        {
            ...
            tup = &catlist->members[i]->tuple;

            /* check it's safe to use in SQL */
            check_safe_enum_use(tup);

            val = NameStr(((Form_pg_enum) GETSTRUCT(tup))->enumlabel);

            cmpLen = strlen(val) > nameLen ? strlen(val) : nameLen;

            if (strncasecmp(val, name, cmpLen) == 0)
            {
                found = true;
