	...
	if (shmid < 0)
	{
		...
		ereport(FATAL,
				(errmsg("could not create shared memory segment: %m"),
				 ...
				 (shmget_errno == EINVAL) ?
				 errhint("This error usually means that Halo's request for a shared memory "
				 ...
				 (shmget_errno == ENOMEM) ?
				 errhint("This error usually means that Halo's request for a shared "
	...
	if (ptr == MAP_FAILED)
	{
		...
		ereport(FATAL,
				(errmsg("could not map anonymous shared memory: %m"),
				 (mmap_errno == ENOMEM) ?
				 errhint("This error usually means that Halo's request "
						 "for a shared memory segment exceeded available memory, "
						 "swap space, or huge pages. To reduce the request size "
						 "(currently %zu bytes), reduce Halo's shared "
