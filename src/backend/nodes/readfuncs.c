...
#define READ_REENTRANT_INT_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
		local_node->fldname = atoi(token);	\
...
#define READ_REENTRANT_UINT_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_UINT64_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);			/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_LONG_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_OID_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_CHAR_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);			/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_ENUM_FIELD(fldname, enumtype) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_FLOAT_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_BOOL_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{ \
		token = pg_strtok(&length);		/* get field value */ \
...
#define READ_REENTRANT_STRING_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		token = pg_strtok(&length);		/* get field value */	\
...
#define READ_REENTRANT_NODE_FIELD(fldname) \
	token = pg_strtok_reentrant(#fldname, &length);		/* skip :fldname */ \
	if (length != -1)	\
	{	\
		(void) token;				/* in case not used elsewhere */ \
...
_readQuery(void)
{
	...
	READ_REENTRANT_NODE_FIELD(mergeActionList);
	READ_REENTRANT_BOOL_FIELD(mergeUseOuterJoin);
...
_readRangeVar(void)
{
	...
	READ_NODE_FIELD(partitionfornode);
	READ_LOCATION_FIELD(location);
...
_readModifyTable(void)
{
	...
	READ_REENTRANT_NODE_FIELD(mergeActionLists);
