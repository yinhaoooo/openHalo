...
_outRangeVar(StringInfo str, const RangeVar *node)
{
	...
	WRITE_NODE_FIELD(partitionfornode);
...
_outFuncCall(StringInfo str, const FuncCall *node)
{
	...
	WRITE_ENUM_FIELD(funcformat, CoercionForm);
	WRITE_LOCATION_FIELD(location);
	WRITE_LOCATION_FIELD(arglocation);
	WRITE_LOCATION_FIELD(endlocation);
}
...
_outValue(StringInfo str, const Value *value)
{
	switch (value->type)
	{
		...
		case T_ByteaString:
            {
                int dataLen = atoi(value->val.str);
                Assert(str != NULL);
                /* Make more room if needed */
                enlargeStringInfo(str, dataLen);
                /* OK, append the data */
                memcpy((str->data + str->len), (value->val.str + 16), dataLen);
                str->len += dataLen;
                str->data[str->len] = '\0';
                break;
            }
