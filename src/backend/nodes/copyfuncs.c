...
_copyRangeVar(const RangeVar *from)
{
	...
	COPY_NODE_FIELD(partitionfornode);
...
_copyFuncCall(const FuncCall *from)
{
	...
	COPY_SCALAR_FIELD(funcformat);
	COPY_LOCATION_FIELD(location);
	COPY_LOCATION_FIELD(arglocation);
	COPY_LOCATION_FIELD(endlocation);
	return newnode;
}
...
_copyValue(const Value *from)
{
	...
	switch (from->type)
	{
		...
		case T_ByteaString:
            if (from->val.str != NULL)
            {
                int fromDataLen = atoi(from->val.str);
                newnode->val.str = palloc(fromDataLen);
                memcpy(newnode->val.str, (from->val.str + 16), fromDataLen);
            }
            else 
            {
                newnode->val.str = (char *)NULL;
            }
			break;
...
static UserVarRef *
_copyUserVarRef(const UserVarRef *from)
{
	UserVarRef   *newnode = makeNode(UserVarRef);

	COPY_STRING_FIELD(userVarName);
	COPY_LOCATION_FIELD(location);
...
static UserVarAssign *
_copyUserVarAssign(const UserVarAssign *from)
{
	UserVarAssign   *newnode = makeNode(UserVarAssign);
...
static SysVarRef *
_copySysVarRef(const SysVarRef *from)
{
	SysVarRef *newnode = makeNode(SysVarRef);

	COPY_STRING_FIELD(sysVarName);
	COPY_LOCATION_FIELD(location);

	return newnode;
}

static MysVariableSetStmt *
_copyMysVariableSetStmt(const MysVariableSetStmt *from)
{
	MysVariableSetStmt *newnode = makeNode(MysVariableSetStmt);

	COPY_NODE_FIELD(varSetStmt);

	return newnode;
}

static MysSelectIntoStmt *
_copyMysSelectIntoStmt(const MysSelectIntoStmt *from)
{
	MysSelectIntoStmt *newnode = makeNode(MysSelectIntoStmt);
...
copyObjectImpl(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_MergeStmt:
			...
			break;
        case T_UserVarRef:
            retval = _copyUserVarRef(from);
            break;
		case T_UserVarAssign:
			retval = _copyUserVarAssign(from);
			break;
        case T_SysVarRef:
            retval = _copySysVarRef(from);
            break;
        case T_MysVariableSetStmt:
            retval = _copyMysVariableSetStmt(from);
			break;
        case T_MysSelectIntoStmt:
            retval = _copyMysSelectIntoStmt(from);
			break;
