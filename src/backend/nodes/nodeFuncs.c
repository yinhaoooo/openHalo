				  ...
				  int flags)
{
	...
	if (walker((Node *) query->mergeActionList, context))	/* merge node */
		return true;
				   ...
				   int flags)
{
	...
	MUTATE(query->mergeActionList, query->mergeActionList, List *);	/* mutate merge node */
						   ...
						   void *context)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_RawStmt:
			{
				if (walker(((RawStmt *) node)->stmt,context))
					return true;
			}
			break;
		...
		case T_CallStmt:
			{
				CallStmt *call = (CallStmt *) node;
				
				if (walker(call->funccall, context))
					return true;
				if (walker(call->funcexpr, context))
					return true;
				if (walker(call->outargs, context))
					return true;
			}
			break;
        ...
        case T_MysVariableSetStmt:
            {
                MysVariableSetStmt *stmt = (MysVariableSetStmt *) node;
                return walker(stmt->varSetStmt, context);
            }
            break;
        
        case T_MysSelectIntoStmt:
            {
                MysSelectIntoStmt *stmt = (MysSelectIntoStmt *) node;
                return walker(stmt->expr, context);
            }
            break;
