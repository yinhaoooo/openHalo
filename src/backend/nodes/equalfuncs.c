...
_equalRangeVar(const RangeVar *a, const RangeVar *b)
{
	...
	COMPARE_NODE_FIELD(partitionfornode);
...
_equalCompositeTypeStmt(const CompositeTypeStmt *a, const CompositeTypeStmt *b)
{
	...
	COMPARE_SCALAR_FIELD(replace);
	COMPARE_SCALAR_FIELD(createFunc);
...
_equalFuncCall(const FuncCall *a, const FuncCall *b)
{
	...
	COMPARE_COERCIONFORM_FIELD(funcformat);
	COMPARE_LOCATION_FIELD(location);
	COMPARE_LOCATION_FIELD(arglocation);
	COMPARE_LOCATION_FIELD(endlocation);
	return true;
}
...
_equalValue(const Value *a, const Value *b)
{
	...
	switch (a->type)
	{
		...
		case T_ByteaString:
            if ((a != NULL) && (b != NULL))
            {
                int aLen = atoi(a->val.str);
                int bLen = atoi(b->val.str);
                if (aLen == bLen)
                {
                    return (memcmp((a->val.str + 16), (b->val.str + 16), aLen) == 0);
                }
                else 
                {
                    return false;
...
static bool
_equalUserVarRef(const UserVarRef *a, const UserVarRef *b)
{
    COMPARE_STRING_FIELD(userVarName);
    COMPARE_LOCATION_FIELD(location);
...
static bool
_equalUserVarAssign(const UserVarAssign *a, const UserVarAssign *b)
{
    COMPARE_STRING_FIELD(userVarName);
	COMPARE_NODE_FIELD(expr);
    COMPARE_LOCATION_FIELD(location);
...
static bool
_equalSysVarRef(const SysVarRef *a, const SysVarRef *b)
{
    COMPARE_STRING_FIELD(sysVarName);
    COMPARE_LOCATION_FIELD(location);
...
static bool
_equalMysVariableSetStmt(const MysVariableSetStmt *a, const MysVariableSetStmt *b)
{
	COMPARE_NODE_FIELD(varSetStmt);

	return true;
}

static bool
_equalMysSelectIntoStmt(const MysSelectIntoStmt *a, const MysSelectIntoStmt *b)
{
...
equal(const void *a, const void *b)
{
	...
	switch (nodeTag(a))
	{
		...
		case T_MergeAction:
			...
			break;
        case T_UserVarRef:
            retval = _equalUserVarRef(a, b);
            break;
		case T_UserVarAssign:
			retval = _equalUserVarAssign(a, b);
            break;
        case T_SysVarRef:
            retval = _equalSysVarRef(a, b);
            break;
        case T_MysVariableSetStmt:
            retval = _equalMysVariableSetStmt(a, b);
			break;
        case T_MysSelectIntoStmt:
            retval = _equalMysSelectIntoStmt(a, b);
			break;
