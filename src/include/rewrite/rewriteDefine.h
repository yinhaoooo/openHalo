...
extern ObjectAddress standard_DefineRule(RuleStmt *stmt, const char *queryString);
extern ObjectAddress standard_DefineQueryRewrite(const char *rulename,
												 Oid event_relid,
												 Node *event_qual,
												 CmdType event_type,
												 bool is_instead,
												 bool replace,
												 List *action);
extern ObjectAddress standard_RenameRewriteRule(RangeVar *relation, const char *oldName,
									   			const char *newName);
extern void standard_setRuleCheckAsUser(Node *node, Oid userid);
extern void standard_EnableDisableRule(Relation rel, const char *rulename,
							  		   char fires_when);
