...
typedef Node *(*CoerceParamHook) (ParseState *pstate, Param *param,
								  ...
								  int location);
typedef Node *(*PreParseFuncCallHook)(ParseState *pstate, FuncCall *cref, List *fargs);
typedef Node *(*PreParseprocedureCallHook)(ParseState *pstate, CallStmt *stmt, List *fargs);
...
struct ParseState
{
	...
	PreParseFuncCallHook p_pre_funccall_hook;
	PreParseprocedureCallHook p_pre_procedurecall_hook;
	void	   *p_ref_hook_state;	/* common passthrough link for above */

	
	bool		p_hasOrphanCols;		/* has orphan columns? */
	

    
    List *targettypelist;               /* for UNION... */
	bool        p_hasmergestmt;             /*防止优化器子查询提升处理*/
...
extern void standard_transformContainerType(Oid *containerType, int32 *containerTypmod);
extern SubscriptingRef *standard_transformContainerSubscripts(ParseState *pstate,
													 		  Node *containerBase,
													 		  Oid containerType,
													 		  int32 containerTypMod,
													 		  List *indirection,
