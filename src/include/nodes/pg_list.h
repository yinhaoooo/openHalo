...
#define llastsecond(l)				lfirst(list_last_second_cell(l))
#define llastsecond_int(l)			lfirst_int(list_last_second_cell(l))
#define llastsecond_oid(l)			lfirst_oid(list_last_second_cell(l))
#define llastsecond_node(type,l)		castNode(type, llastsecond(l))
...
/*
 * Return the last second cell in a non-NIL List.
 */
static inline ListCell *
list_last_second_cell(const List *list)
{
	Assert(list != NIL && list_length(list) >= 2);
	return &list->elements[list->length - 2];
}
...
extern List *list_union_all(const List *list1, const List *list2);
