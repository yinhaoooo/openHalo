...
if test "$BISON"; then
  ...
  then
    AC_MSG_WARN([
*** The installed version of Bison, $BISON, is too old to use with Halo.
...
if test -z "$BISON"; then
  AC_MSG_WARN([
*** Without Bison you will not be able to build Halo from Git nor
*** change any of the parser definition files.  You can obtain Bison from
*** a GNU mirror site.  (If you are using the official distribution of
*** Halo then you do not need to worry about this, because the Bison
      ...
      then
        ...
        if $pgac_candidate -t conftest.l 2>/dev/null | grep FLEX_SCANNER >/dev/null 2>&1; then
          ...
          then
            ...
            AC_MSG_WARN([
*** The installed version of Flex, $pgac_candidate, is too old to use with Halo.
...
if test x"$pgac_cv_path_flex" = x"no"; then
  AC_MSG_WARN([
*** Without Flex you will not be able to build Halo from Git nor
*** change any of the scanner definition files.  You can obtain Flex from
*** a GNU mirror site.  (If you are using the official distribution of
*** Halo then you do not need to worry about this because the Flex
*** output is pre-generated.)])
...
# PGAC_LDAP_SAFE
# --------------
# Halo sometimes loads libldap_r and plain libldap into the same
